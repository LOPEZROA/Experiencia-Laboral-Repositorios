...
def _header_names_from_values(header_vals: List[Any]) -> List[str]:
    names: List[str] = []
    used: Dict[str, int] = {}
...
class DBIndex:
    def __init__(self, db_dir: Path):
        ...
        self.historico_search_rows: List[Dict[str, Any]] = []
        self.historico_search_by_rut: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        self.historico_search_by_id: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        ...
        self.nomina_search_rows: List[Dict[str, Any]] = []
        self.nomina_search_by_rut: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        self.nomina_search_by_id: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    ...
    def _load_historico(self) -> None:
        ...
        header_vals = [c.value for c in header_cells]
        header_names = _header_names_from_values(header_vals)
        ...
        for r in ws.iter_rows(min_row=header_row + 1, values_only=True):
            try:
                ...
                rut_norm = f"{run}-{dv}" if run and dv else ""
                row_search = {header_names[i]: _get(i + 1) for i in range(len(header_names))}
                row_search["ORIGEN_BASE"] = "Historico Minsal"
                row_search["ORIGEN_ARCHIVO"] = path.name
                row_search["ORIGEN_HOJA"] = ws.title
                row_search["RUT_ENCONTRADO"] = rut_norm
                row_search["ID_LOCAL_ENCONTRADO"] = id_local
                row_search["SIGTE_ID_ENCONTRADO"] = sigte_id
                self.historico_search_rows.append(row_search)
                if rut_norm:
                    self.historico_search_by_rut[rut_norm].append(row_search)
                if id_local:
                    self.historico_search_by_id[id_local].append(row_search)
                if sigte_id and sigte_id != id_local:
                    self.historico_search_by_id[sigte_id].append(row_search)
            except Exception:
                continue
        ...
        def consume_rows(
            ...
            for row in rows:
                ...
                if has_val:
                    ...
                    src_sheet = ""
                    if ":" in source:
                        src_file, src_sheet = source.split(":", 1)
                    row_data["ORIGEN_BASE"] = "Nominas"
                    row_data["ORIGEN_ARCHIVO"] = src_file
                    row_data["ORIGEN_HOJA"] = src_sheet
        ...
        if path.suffix.lower() == ".xlsb":
            ...
            try:
                ...
                for sheet_name, sheet_label in sheets_to_read:
                    ...
                    header_vals = df_raw.iloc[header_row - 1].tolist()
                    header_names = _header_names_from_values(header_vals)
    ...
    def _rebuild_nomina_aggregates(self) -> None:
        ...
        self.nomina_search_rows = []
        self.nomina_search_by_rut = defaultdict(list)
        self.nomina_search_by_id = defaultdict(list)
        ...
        for _mtime, _size, _name, key in file_items:
            ...
            for row in data.rows:
                run = normalize_run(row.get("RUN", ""))
                dv = normalize_dv(row.get("DV", ""))
                rut_norm = ""
                for rut_field in ("RUT CONCATENADO", "RUTCONCATENADO", "RUT"):
                    if rut_field in row:
                        rut_norm = normalize_rut_concat(row.get(rut_field))
                        if rut_norm:
                            break
                if not rut_norm and run and dv:
                    rut_norm = f"{run}-{dv}"

                id_local = normalize_id(row.get("ID_LOCAL", ""))
                sigte_id = normalize_id(row.get("SIGTE_ID", ""))

                search_row = dict(row)
                search_row["RUT_ENCONTRADO"] = rut_norm
                search_row["ID_LOCAL_ENCONTRADO"] = id_local
                search_row["SIGTE_ID_ENCONTRADO"] = sigte_id
                self.nomina_search_rows.append(search_row)
                if rut_norm:
                    self.nomina_search_by_rut[rut_norm].append(search_row)
                if id_local:
                    self.nomina_search_by_id[id_local].append(search_row)
                if sigte_id and sigte_id != id_local:
                    self.nomina_search_by_id[sigte_id].append(search_row)
    ...
    def reload_historico(self) -> None:
        ...
        self.historico_search_rows = []
        self.historico_search_by_rut = defaultdict(list)
        self.historico_search_by_id = defaultdict(list)
        self._load_historico()
...
def build_local_system_records(path: Path) -> Dict[str, Dict[str, str]]:
    df = load_work_df(path)
    if df is None or df.empty:
        return {}

    col_id = pick_col(df, ["ID_LOCAL", "id_local", "SIGTE_ID", "sigte_id"])
    if not col_id:
        return {}

    verify_cols = {field: pick_col(df, candidates) for field, candidates in VERIFY_FIELDS.items()}
    best_records: Dict[str, Tuple[int, Dict[str, str]]] = {}

    for i in range(len(df)):
        id_key = normalize_id(df[col_id].iat[i])
        if not id_key:
            continue
        rec: Dict[str, str] = {}
        filled = 0
        for field in VERIFY_FIELDS.keys():
            col = verify_cols.get(field)
            val = normalize_compare_value(field, df[col].iat[i]) if col else ""
            rec[field] = val
            if val:
                filled += 1
        prev = best_records.get(id_key)
        if prev is None or filled > prev[0]:
            best_records[id_key] = (filled, rec)

    return {k: v[1] for k, v in best_records.items()}
...
def process_file(
    ...
    if selected.get("defunciones", False):
        ...
        df["FECHA DEFUNCION"] = fecha_def_vals.map(lambda d: d.isoformat() if d else "")
...
def _refresh_db_if_needed() -> None:
...
def _run_local_system_job(job_id: str, work_path: Path, local_path: Path) -> None:
    _update_job(job_id, status="running", progress=0, error="")
    t0 = time.perf_counter()
    out_path: Optional[Path] = None
    try:
        ...
        db = get_db()
        _raise_if_cancel_requested(job_id)
        local_records = build_local_system_records(local_path)
        selected_all = {
            "historico": True,
            "nominas": True,
            "verificacion": True,
        ...
        out_path, _elapsed = process_file(
            ...
            db,
            progress_cb=cb,
            local_system_records=local_records,
            cancel_cb=lambda: _raise_if_cancel_requested(job_id),
        )
        _raise_if_cancel_requested(job_id)
        elapsed_total = time.perf_counter() - t0
...
def _run_busqueda_job(job_id: str, search_rut: str, search_id_local: str) -> None:
    _update_job(job_id, status="running", progress=0, error="", busqueda_stats=None)
    out_path: Optional[Path] = None
    try:
        _raise_if_cancel_requested(job_id)
        db = get_db()
        ...
        out_path, stats, elapsed = process_busqueda_sudais(
            search_rut,
            search_id_local,
            db=db,
            progress_cb=cb,
...
def _build_busqueda_view_df(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=[
            "ORIGEN_BASE", "ORIGEN_ARCHIVO", "ORIGEN_HOJA",
            "RUT_ENCONTRADO", "ID_LOCAL_ENCONTRADO", "SIGTE_ID_ENCONTRADO",
            "PRESTA_MIN", "TIPO_PREST", "PRESTA_EST", "F_ENTRADA", "F_SALIDA", "ESTAB_DEST"
        ])

    out = df.copy()
    col_rut = pick_col(out, ["RUT CONCATENADO", "RUTCONCATENADO", "RUT", "rut"])
    col_run = pick_col(out, ["RUN", "run"])
    col_dv = pick_col(out, ["DV", "dv"])
    col_id = pick_col(out, ["ID_LOCAL", "id_local"])
    col_sigte = pick_col(out, ["SIGTE_ID", "sigte_id"])

    if col_rut and col_rut in out.columns:
        rut_vals = out[col_rut].map(normalize_rut_concat)
    elif col_run and col_dv and col_run in out.columns and col_dv in out.columns:
        rut_vals = out[col_run].map(normalize_run) + "-" + out[col_dv].map(normalize_dv)
    else:
        rut_vals = pd.Series([""] * len(out), index=out.index)

    id_vals = out[col_id].map(normalize_id) if col_id and col_id in out.columns else pd.Series([""] * len(out), index=out.index)
    sigte_vals = out[col_sigte].map(normalize_id) if col_sigte and col_sigte in out.columns else pd.Series([""] * len(out), index=out.index)

    out["RUT_ENCONTRADO"] = rut_vals
    out["ID_LOCAL_ENCONTRADO"] = id_vals
    out["SIGTE_ID_ENCONTRADO"] = sigte_vals
    ...
    for cands in [
        ...
        if col and col in out.columns and col not in view_cols:
            view_cols.append(col)
    return out[view_cols].copy()


def _search_busqueda_sudais(search_rut: str, search_id_local: str, db: Optional[DBIndex] = None) -> Tuple[pd.DataFrame, str]:
    rut_norm = normalize_rut_concat(search_rut) if (search_rut or "").strip() else ""
    id_norm = normalize_id(search_id_local) if (search_id_local or "").strip() else ""
    if not rut_norm and not id_norm:
        return pd.DataFrame(), ""
    if rut_norm and id_norm:
        tipo = "RUT + ID_LOCAL (coincidencia obligatoria)"
    elif rut_norm:
        tipo = "RUT"
    else:
        tipo = "ID_LOCAL"
    ...
    rows: List[Dict[str, Any]] = []
    if rut_norm and id_norm:
        # Si vienen ambos campos, ambos deben coincidir en el mismo caso.
        candidate_rows: List[Dict[str, Any]] = []
        candidate_rows.extend(db.historico_search_by_id.get(id_norm, []))
        candidate_rows.extend(db.nomina_search_by_id.get(id_norm, []))
        for row in candidate_rows:
            row_rut = normalize_rut_concat(row.get("RUT_ENCONTRADO", ""))
            row_id_local = normalize_id(row.get("ID_LOCAL_ENCONTRADO", ""))
            row_sigte = normalize_id(row.get("SIGTE_ID_ENCONTRADO", ""))
            if row_rut == rut_norm and (row_id_local == id_norm or row_sigte == id_norm):
                rows.append(row)
    else:
        if rut_norm:
            rows.extend(db.historico_search_by_rut.get(rut_norm, []))
            rows.extend(db.nomina_search_by_rut.get(rut_norm, []))
        if id_norm:
            rows.extend(db.historico_search_by_id.get(id_norm, []))
            rows.extend(db.nomina_search_by_id.get(id_norm, []))

    if not rows:
        return pd.DataFrame(), tipo

    dedup: List[Dict[str, Any]] = []
    seen_ref: set = set()
    for row in rows:
        row_ref = id(row)
        if row_ref in seen_ref:
            continue
        seen_ref.add(row_ref)
        dedup.append(dict(row))
    out = pd.DataFrame(dedup)
    return out, tipo


def process_busqueda_sudais(
    search_rut: str,
    search_id_local: str,
    db: Optional[DBIndex] = None,
    progress_cb: Optional[Callable[[int], None]] = None,
    ...
    t0 = time.perf_counter()
    report(5)
    results_df, search_tipo = _search_busqueda_sudais(search_rut, search_id_local, db=db)
    report(75)

    out_name = f"LE_NOGES_busqueda_sudais_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex}.xlsx"
    out_path = OUTPUT_DIR / out_name
    if results_df.empty:
        export_df = pd.DataFrame([{
            "BUSQUEDA_RUT": search_rut,
            "BUSQUEDA_ID_LOCAL": search_id_local,
            "TIPO_BUSQUEDA": search_tipo or "No identificado",
            "RESULTADO": "Sin coincidencias en Nominas ni Historico Minsal",
        }])
    else:
        export_df = results_df.copy()
    export_df.to_excel(out_path, index=False, sheet_name="Busqueda")

    view_df = _build_busqueda_view_df(results_df)
    rows_total = int(len(view_df))
    preview_df = view_df.head(TABLE_PREVIEW_LIMIT).copy() if not view_df.empty else view_df
    columns = [str(c) for c in preview_df.columns] if not preview_df.empty else []
    rows = preview_df.fillna("").astype(str).to_dict(orient="records") if not preview_df.empty else []

    elapsed = time.perf_counter() - t0
    report(100)
    stats = {
        "search_rut": search_rut,
        "search_id_local": search_id_local,
...
CATEGORIES = {
    ...
    "busqueda-sudais": {
        "title": "Busqueda Sudais",
        "color": "verde-metalizado",
        "options": [],
    },
    "cruce-sistema-local": {
        "title": "Cruce sistema local",
...
def cruce_sistema_local():
    if request.method == "POST":
        if not _can_start_new_job():
            flash(f"Hay {MAX_CONCURRENT_JOBS} procesos en ejecución. Espera a que termine uno para iniciar otro.")
            return render_template("cruce_sistema_local.html", title=APP_TITLE, cat=CATEGORIES["cruce-sistema-local"])

        work_file = request.files.get("workfile")
        local_file = request.files.get("localfile")
        if not work_file or work_file.filename == "":
            flash("Debes cargar el archivo de trabajo.")
            return render_template("cruce_sistema_local.html", title=APP_TITLE, cat=CATEGORIES["cruce-sistema-local"])
        if not local_file or local_file.filename == "":
            flash("Debes cargar el archivo de Sistema local.")
            return render_template("cruce_sistema_local.html", title=APP_TITLE, cat=CATEGORIES["cruce-sistema-local"])

        if not allowed_file(work_file.filename) or not allowed_file(local_file.filename):
            flash("Formato inválido. Solo se permite .xlsx o .xlsb")
            return render_template("cruce_sistema_local.html", title=APP_TITLE, cat=CATEGORIES["cruce-sistema-local"])

        work_name = secure_filename(work_file.filename)
        local_name = secure_filename(local_file.filename)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        work_path = UPLOAD_DIR / f"LE_NOGES_upload_{ts}_{uuid.uuid4().hex}_{work_name}"
        local_path = UPLOAD_DIR / f"LE_NOGES_upload_{ts}_{uuid.uuid4().hex}_{local_name}"
        ...
        _init_job(
            job_id,
            owner_rut=_current_user_rut(),
            input_file=f"{work_path}|{local_path}",
            repeat_url=url_for("categoria", slug="cruce-sistema-local"),
        )
        thread = threading.Thread(
            target=_run_local_system_job,
            args=(job_id, work_path, local_path),
            daemon=True,
        )
        thread.start()
...
def busqueda_sudais():
    if request.method == "POST":
        search_rut = (request.form.get("search_rut", "") or "").strip()
        search_id_local = (request.form.get("search_id_local", "") or "").strip()
        if not search_rut and not search_id_local:
            flash("Debes ingresar al menos un campo: RUT o ID_LOCAL.")
            return render_template("busqueda_sudais.html", title=APP_TITLE)

        job_id = uuid.uuid4().hex
        _init_job(
            job_id,
            owner_rut=_current_user_rut(),
            input_file=f"{search_rut}|{search_id_local}",
            repeat_url=url_for("categoria", slug="busqueda-sudais"),
        )
        thread = threading.Thread(
            target=_run_busqueda_job,
            args=(job_id, search_rut, search_id_local),
            daemon=True,
        )
        thread.start()
        ...
        return render_template(
            ...
            title=APP_TITLE,
            job_id=job_id,
            redirect_url=url_for("busqueda_sudais_result", job_id=job_id),
        )
...
def categoria(slug: str):
    ...
    if slug == "cruce-sistema-local":
        return cruce_sistema_local()
...
@app.get("/busqueda_sudais_result/<job_id>")
def busqueda_sudais_result(job_id: str):
    ...
    stats = job.get("busqueda_stats") if isinstance(job.get("busqueda_stats"), dict) else {}
    return render_template(
        "busqueda_sudais.html",
        title=APP_TITLE,
        search_rut=stats.get("search_rut", ""),
        search_id_local=stats.get("search_id_local", ""),
        search_tipo=stats.get("search_tipo", ""),
        rows=stats.get("rows", []),
